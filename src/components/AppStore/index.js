...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredApp = newFilteredAppList.filter(eachAppList =>
      eachAppList.appName.toLowerCase().includes(userInput.toLowerCase()),
    )
        ...
        <ul className="btn-container">
          {tabsList.map(eachTab => (
            <TabItem
              eachTab={eachTab}
              key={eachTab.tabId}
              filterCategoryApp={this.filterCategoryApp}
            />
          ))}
        </ul>
